...
- 性能指标
  ...
  4. 时延(delay/latency)=发送/传输时延+传播时延+处理时延+排队时延:
...
什么是码元：码元(code)——在使用时间域（或简称为时域）的波形表示数字信号时，代表不同离散数值的基本波形。
...
```
规格化内积：就是内积结果再除以向量的维数，如向量s（1，2，3）点乘向量t（7，8，9）的规格化内积为（1*7+2*8+3*9）/3=50/3 这个两个都是三维 向量。
```
...
参考链接：[链接](https://www.jianshu.com/p/7f12b58b0b44)

计算方法:

`预定义的生成多项式P(X)=X^3+X^2+1` n=3,看最大的幂 
...
![image-20200814155131523](C:\Users\蔡建斌\AppData\Roaming\Typora\typora-user-images\image-20200814155131523.png)

![image-20200814155225028](C:\Users\蔡建斌\AppData\Roaming\Typora\typora-user-images\image-20200814155225028.png)

![image-20200814155304434](C:\Users\蔡建斌\AppData\Roaming\Typora\typora-user-images\image-20200814155304434.png)
